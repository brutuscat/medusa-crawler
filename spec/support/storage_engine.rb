...
RSpec.shared_examples_for "storage engine" do
  ...
  it "should implement keys" do
    ...
    pages = urls.map { |url| Medusa::Page.new(URI(url)) }
  ...
  it "should implement merge!, and return self" do
    ...
    hash = {SPEC_DOMAIN => Medusa::Page.new(URI(SPEC_DOMAIN)),
            SPEC_DOMAIN + 'test' => Medusa::Page.new(URI(SPEC_DOMAIN + 'test'))}
