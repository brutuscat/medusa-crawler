...
To crawl a private site that uses a server-rendered login form, submit the form first and pass the
session cookie to Medusa:

    MEDUSA_USERNAME=user MEDUSA_PASSWORD=secret ruby examples/private_site_with_login.rb

See +examples/private_site_with_login.rb+ for the complete example. It uses a regular HTTP form
POST; sites that require JavaScript or a more involved CSRF flow need site-specific handling before
starting Medusa.

The release smoke test in +spec/smoke/private_site_with_login_smoke.rb+ runs the example against a
public test login and extracts text from its authenticated page.
